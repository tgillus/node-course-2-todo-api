...
describe("POST /users", () => {
  ...
  it("should return validation errors if request invalid", done => {
    request(app)
      .post("/users")
      .send({ email: "tramaine", password: "abc" })
      .expect(400)
      .end(done);
  });
...
describe("POST /users/login", () => {
  ...
  it("should reject request when user does not exist", done => {
    request(app)
      .post("/users/login")
      .send({ email: "bogus@bogus.com", password: "bogusPassword" })
      .expect(400)
      .expect(res => {
        expect(res.headers["x-auth"]).toBeFalsy();
      })
